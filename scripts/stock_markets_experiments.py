...
def get_combinations() -> tuple[list[dict], list[dict]]:
    ...
    combinations_name = [
        dict(zip(keys_name, combination))
        for combination in itertools.product(*values_name)
    ]
...
def stock_markets_experiments(args):
    ...
    for combination, combination_name in tqdm(
        zip(combinations, combinations_name), leave=False
    ):
        ...
        for stock_1 in DATA_OF_STOCKS:
            ...
            for stock_2 in DATA_OF_STOCKS:
                row.append(
                    combination["correlation_network"](
                        data_1=stock_1.returns, data_2=stock_2.returns
                    )
                )
            correlation_matrix.append(row)
        true_num_clusters, true_labels = combination["clustering_method"](
            np.array(correlation_matrix), combination["number_clusters"]
        )
        ...
        combination_name["cluster_distribution"] = (
            true_num_clusters,
            cluster_distribution,
        )
        results_experiments = results_experiments._append(
            pd.Series(combination_name), ignore_index=True
        )
