...
class ClusteringMethods:
    ...
    def _get_community_labels(
        self, number_nodes: int, communities: np.array
    ) -> np.array:
        """
    ...
    def louvain_clustering(self, adj_matrix: np.array, _num_clusters: int) -> np.array:
        ...
        if (adj_matrix < 0).any():
            raise ValueError(
                "Adjacency matrix contains negative weights, which are not supported."
            )
    ...
    def normalized_spectral_clustering(
        self, adj_matrix: np.array, num_clusters: int
    ) -> np.array:
        """
    ...
    def single_clustering(self, adj_matrix: np.array, num_clusters: int) -> np.array:
        ...
        return num_clusters, self._get_community_labels(
            graph.number_of_nodes(), result_communities
        )
    ...
    def ward_clustering(self, adj_matrix: np.array, num_clusters: int) -> np.array:
        """
        Cluster objects using a correlation matrix with Ward's hierarchical method.

        Parameters:
        adj_matrix (np.ndarray): Square correlation matrix (n x n)
        threshold (float): Threshold to form flat clusters (optional)
        num_clusters (int): Number of clusters to form (optional)

        Returns:
        np.ndarray: Array of cluster labels
        """

        distance_matrix = np.sqrt(2 * (1 - adj_matrix))
        condensed_dist = squareform(distance_matrix, checks=False)
        Z = linkage(condensed_dist, method="ward")
        return num_clusters, fcluster(Z, num_clusters, criterion="maxclust")
