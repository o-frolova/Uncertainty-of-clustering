        ...
        for multi_distribution in PARAMS_NAME["multivariate_distribution"]:
            for corr_network in PARAMS_NAME["correlation_network"]:
                for size_samples in PARAMS_NAME["sample_size_of_observations"]:
                    ...
                    filtered_data = data[
                        (data["correlation_network"] == corr_network)
                    ...
                    filtered_data = filtered_data[
                        filtered_data["clustering_method"].isin(clustering_order)
                    ]
                    ...
                    filtered_data["cluster_distribution"] = filtered_data[
                    ...
                    info_stat = filtered_data["cluster_distribution"].map(
                        get_statistics
                    )

                    filtered_data = filtered_data.drop(["cluster_distribution"], axis=1)
                    result_louvain = pd.concat(
                        [
                            filtered_data,
                            info_stat.apply(
                                lambda x: (
                                    x.squeeze() if isinstance(x, pd.DataFrame) else x
                                )
                            ),
                        ],
                        axis=1,
                    )
                    result_louvain = result_louvain.drop(
                        [
                            "Unnamed: 0",
                            "count",
                            "correlation_network",
