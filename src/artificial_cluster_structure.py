...
class ArtificialСlusterStructure:
    ...
    def _generated_data(
        ...
        generated_vectors = multivariate_distribution(
        ...
        for random_value_index in range(len(generated_vectors[0])):
            observations_random_variables.append(
                generated_vectors[:, random_value_index]
            )

        return np.array(observations_random_variables)

    def _correlation_estimation(
        self,
        ...
        for observation_random_variable_1 in observations_random_variables:
            ...
            for observation_random_variable_2 in observations_random_variables:
                row.append(
                    correlation_method(
                        observation_random_variable_1, observation_random_variable_2
                    )
                )
    ...
    def clustering(
        ...
        observations_random_variables = self._generated_data(
            multivariate_distribution=multivariate_distribution,
            mean_vector=mean_vector,
            cov_matrix=cov_matrix,
            sample_size_of_observations=sample_size_of_observations,
        )
        correlation_estimation = self._correlation_estimation(
            observations_random_variables=observations_random_variables,
            correlation_method=correlation_method,
        )

        return self._clustering(
            correlation_estimation=correlation_estimation,
